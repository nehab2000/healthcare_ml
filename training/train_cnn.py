...
Trains a CNN model (ResNet50, DenseNet121, or EfficientNet) for pneumonia detection.

Usage:
    python training/train_cnn.py

The model architecture and training settings can be configured in config/model_config.yaml.
Best model is automatically saved to checkpoints/cnn/best_model.pth
...
def main():
    ...
    print(f"Model created: {model_config['cnn']['architecture']}")
    ...
    # TensorBoard
    log_dir = Path(model_config['logging']['log_dir']) / 'cnn'
    writer = SummaryWriter(log_dir)
    
    # Checkpoint directory
    checkpoint_dir = Path(model_config['checkpoint']['save_dir']) / 'cnn'
